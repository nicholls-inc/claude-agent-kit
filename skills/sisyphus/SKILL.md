...
Set runtime `activePersona` to `sisyphus` in `.agent-kit/state/runtime.local.json` using inline execution.
...
4. Confirm persona switch and mention you can switch again with `/claude-agent-kit:hephaestus`, `/claude-agent-kit:prometheus`, or `/claude-agent-kit:atlas`.
