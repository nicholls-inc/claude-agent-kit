...
## Execute
1. Ensure `.agent-kit/state/` exists.
2. Read `.agent-kit/state/runtime.local.json` (or `{}` if missing).
3. Set:
   - `version=1`
...
5. Confirm with a short response that ultrawork is enabled and remind user about `/claude-agent-kit:stop-continuation`.

## Constraints
- Never attempt hidden model override.
- Keep this inline (non-fork skill).
- Use `.agent-kit/state/runtime.local.json` as state of truth.
